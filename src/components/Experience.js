export default function Experience() {
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-white/50 to-white/30 backdrop-blur-sm">
            ...
            <div className="space-y-8">
              {items.map((item, index) => (
                <div key={index} className="relative pl-0 md:pl-20">
                  {/* Timeline dot */}
                  <div className="absolute left-6 top-6 w-4 h-4 rounded-full bg-white border-4 border-primary-500 shadow-lg hidden md:block z-10"></div>
                  
                  <div className="glass rounded-2xl p-6 md:p-8 card-hover">
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                      <div className="flex-1">
                        <div className={`inline-block px-4 py-1 rounded-full text-white text-sm font-semibold mb-3 bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 shadow-lg`}>
                          {item.period}
                        </div>
                        <h3 className="text-2xl font-bold text-gray-800 mb-1">{item.company}</h3>
                        <p className="text-lg font-semibold text-primary-600 mb-2">{item.title}</p>
